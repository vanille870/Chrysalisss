...
public class BreakableObjectSpawner : MonoBehaviour, IBreakable
{
    ...
    {
        particleSystemPoolManager.GetParticleSystemGO(particlSysLocation, gameObject);
    }

    public void ParticleSysTimer()
    {
        if (hasParticleSys == true)
        {
            particleSysTimer += Time.deltaTime;

            if (particleSysTimer >= maxParticleSytemTimer)
            {
                particleSystemPoolManager.ReturnParticleSytem(particlSysName);
                hasParticleSys = false;
                particleSysTimer = 0;
            }
        }
    }
